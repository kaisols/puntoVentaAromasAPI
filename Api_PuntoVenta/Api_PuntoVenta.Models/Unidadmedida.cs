...
namespace Api_PuntoVenta.Models
{
    ...
    public class Unidadmedida
    {
        ...
        public Respuesta ConsultaMasiva()
        {
            ...
            try
            {
                List<Unidadmedida> objEncontrado = null;
                string consulta = "";
                ...
                if (objDatos.AbrirConexion())
                {
                    if (tipoconsulta == 1)
                    {
                        consulta = $@"SELECT id, nombre, simbolo, fecha_registro, estado
                                      FROM T_UnidadMedida
                                      WHERE estado = 1
                                      FOR JSON PATH";
                    }

                    objEncontrado = JsonConvert.DeserializeObject<List<Unidadmedida>>(objDatos.HacerSelectJSONPATH(consulta));
