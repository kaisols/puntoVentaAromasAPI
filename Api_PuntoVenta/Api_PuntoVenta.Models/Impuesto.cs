...
namespace Api_PuntoVenta.Models
{
    ...
    public class Impuesto  
    {
        ...
        public Respuesta ConsultaMasiva()
        {
            ...
            try
            {
                List<Impuesto> objEncontrado = null;
                string consulta = "";
                ...
                if (objDatos.AbrirConexion())
                {
                    if (tipoconsulta == 1)
                    {
                        consulta = $@"SELECT id, nombre, PorcentajeIVA, fecha_registro, estado 
                                        FROM T_Impuesto
                                        WHERE estado = 1
                                    FOR JSON PATH";
                    }

                    objEncontrado = JsonConvert.DeserializeObject<List<Impuesto>>(objDatos.HacerSelectJSONPATH(consulta));
