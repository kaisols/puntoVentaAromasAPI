...
namespace Api_PuntoVenta.Models
{
    ...
    public class Tipofactura  
    {
        ...
        public Respuesta ConsultaMasiva()
        {
            ...
            try
            {
                List<Tipofactura> objEncontrado = null;
                string consulta = "";
                ...
                if (objDatos.AbrirConexion())
                {
                    if (tipoconsulta == 1)
                    {
                        consulta = $@"SELECT id, nombre, fecha_registro, estado 
                                    FROM T_TipoFactura WHERE estado = 1 
                                    FOR JSON PATH";
                    }

                    objEncontrado = JsonConvert.DeserializeObject<List<Tipofactura>>(objDatos.HacerSelectJSONPATH(consulta));
