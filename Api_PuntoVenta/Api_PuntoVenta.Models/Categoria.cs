...
{
    [ValidateNever]
    public class Categoria : IOrdenConsulta
    {
        #region ============================= PROPIEDADES =============================
        ...
        public Respuesta Actualizar()
        {
            ...
            try
            {
                ...
                {
                    consulta = $@"
                                   UPDATE T_Categoria SET nombre = '{this.nombre}' WHERE id = {id};
                ...
                else if (tipoconsulta == 2)
                {
                    ...
                    consulta = $@"UPDATE T_Categoria SET estado = '{this.estado}' WHERE id = '{this.id}'
                                            {miAuditoria.ObtenerAuditoria()}                                
                                            SELECT 2;";
                }
        ...
        public Respuesta ConsultaMasiva()
        {
            ...
            try
            {
                List<Categoria> objEncontrado = null;
                string consulta = "";
                ...
                if (objDatos.AbrirConexion())
                {
                    if (tipoconsulta == 1)
                    {
                        consulta = $@"SELECT id, nombre, fecha_registro, estado 
                                    FROM T_Categoria 
                                    FOR JSON PATH";
                    }

                    objEncontrado = JsonConvert.DeserializeObject<List<Categoria>>(objDatos.HacerSelectJSONPATH(consulta));
        ...
        public Respuesta Guardar()
        {
            ...
            try
            {
                ...
                if (tipoconsulta == 1)
                {
                    consulta = $@"IF((SELECT COUNT(*) FROM T_Categoria WHERE nombre = '{nombre}') <= 0)
	                                BEGIN
                                        
                                        DECLARE @id as int = (SELECT ISNULL(MAX(id), 0) + 1 from T_Categoria);

		                                INSERT INTO T_Categoria (id, nombre)
		                                VALUES(@id, '{nombre}'); 
        ...
        public Respuesta Obtener()
        {
            ...
            try
            {
                ...
                if (objDatos.AbrirConexion())
                {
                    ...
                    objEncontrado = JsonConvert.DeserializeObject<Categoria>(objDatos.HacerSelectJSONPATH(consulta));
